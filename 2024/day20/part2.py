...
def cheat(grid, row, col):
    for dr, dc in product(range(-20, 21), range(-20, 21)):
        rr, cc = row + dr, col + dc
        if abs(row - rr) + abs(col - cc) <= 20 and in_bounds(grid, rr, cc) and grid[rr][cc] != '#':
            yield (rr, cc)
    ...
    for j in range(len(f[0])):
        if f[i][j] != '#':
            for a, b in cheat(f, i, j):
                if time - (from_start[i][j] + from_end[a][b] + abs(i - a) + abs(j - b)) >= 100:
                    ans += 1
print(ans)