...
let solve = 
  ...
  | x :: xs ->
    ...
    aux xs
  in
  Hashtbl.replace dp "svr" 1;
  aux order;
  let fft = Hashtbl.find_opt dp "fft" |> Option.value ~default:0 in
  Hashtbl.clear dp;
  Hashtbl.replace dp "fft" fft;
  aux order;
  let dac = Hashtbl.find_opt dp "dac" |> Option.value ~default:0 in
  Hashtbl.clear dp;
  Hashtbl.replace dp "dac" dac;
  aux order;
